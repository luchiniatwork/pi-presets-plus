...
- **WHEN** apply runs for a preset with `thinkingLevel: "max"` and the resolved model has `reasoning: true` and no `thinkingLevelMap` field
- **THEN** `pi.setThinkingLevel("off")` SHALL be called
- **AND** an info notification SHALL be emitted naming the preset, the requested level (`max`), and the actual level (`off`)
...
- **WHEN** apply runs for a preset with `thinkingLevel: "xhigh"` and the resolved model has `reasoning: true` and `thinkingLevelMap: { "xhigh": "max" }`
...
- **WHEN** apply runs for a preset with `thinkingLevel: "max"` and the resolved model has `reasoning: true` and `thinkingLevelMap: { "max": "max" }`
- **THEN** `pi.setThinkingLevel("max")` SHALL be called and no clamp notification SHALL be emitted
