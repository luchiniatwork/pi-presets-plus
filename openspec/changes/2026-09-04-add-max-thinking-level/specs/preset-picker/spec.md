...
The Thinking value for level `"max"` SHALL render with the `thinkingMax` theme color. Because that color only exists in Pi versions whose themes define it (Pi >= 0.80.6) and `fg()` throws on unknown theme colors, the card SHALL fall back to rendering `"max"` with the `thinkingXhigh` color when `thinkingMax` is unavailable, matching Pi's own fallback. The fallback SHALL apply only to the `"max"` level; other levels render with their existing colors on all supported Pi versions.
...
#### Scenario: Max level renders with thinkingMax color

- **WHEN** the picker renders a preset with `thinkingLevel: "max"` on a Pi version whose theme defines `thinkingMax`
- **THEN** the Thinking value SHALL read `Max` and SHALL render in the `thinkingMax` color

#### Scenario: Max level falls back to thinkingXhigh color

- **WHEN** the picker renders a preset with `thinkingLevel: "max"` on a Pi version whose theme does not define `thinkingMax`
- **THEN** the Thinking value SHALL read `Max` and SHALL render in the `thinkingXhigh` color
- **AND** the render SHALL NOT throw
